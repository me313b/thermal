...
- FEA tab: the COMSOL model file is now the first, primary download; FEMM is labelled as the free cross-check of the same geometry. A caption under the buttons gives the exact comsolbatch run line.
- Every COMSOL Java file now ships a ready-made **parametric sweep** at the study node (over the governing film coefficient: h_side, h_oil or h_cell per case), commented out so the base run stays fast - uncomment three lines and COMSOL tabulates every evaluation row per swept value.
- The samples package is restructured COMSOL-first: comsol/ and femm/ folders plus a README.txt with run instructions for both tools.

## v10.5

FEA on the ladder: parametric 2D models for FEMM and COMSOL, generated by a new visible FEA tab.

- **New FEA tab** (after Cases): pick one of four 2D models - heated cell (axisymmetric, anisotropic core, optional can layer and end cooling), one annular fin (axisymmetric half-pitch slice), plate on tube (planar, per metre), and the full serpentine unit cell (cell slab + films + plate + tube). For each: adjust every input, see a drawn preview of the exact FEA domain with its boundary conditions, read the operating point the app computes (all film h values and its predicted temperatures and efficiencies), then download a FEMM heat-flow Lua script and a COMSOL 6.x Java model file, both always visible, no build step.
- **Division of labour, stated in every file:** FEMM/COMSOL heat transfer solve conduction; the films enter as named convection parameters computed by the app's own correlations and remain editable in the tools. The FEA therefore independently checks what the app approximates inside the solid: core anisotropy, fin and plate efficiency, wall conduction, 2D spreading. Each file embeds the app's predictions next to unambiguous probes; the COMSOL model additionally builds Maximum/Average/Integration operators, an Evaluation Group with FEA values, app values and deltas, writes a results text file, saves an .mph, and sets up a temperature surface + contour plot. FEMM scripts probe named points, integrate the heat into the water, write a results text file and show a density plot.
- **fea_check.py**: the generated Lua is EXECUTED under lua5.4 with every hi_/ho_ call stubbed, and the generated Java is COMPILED with javac against a minimal chainable COMSOL API stub - so syntax, undefined names and arity errors fail here rather than on your machine. Smoke generates all eight files and asserts their markers. Honesty note: no FEMM or COMSOL licence exists in the build environment; if either tool rejects a call, report it and it is a one-round fix.
- New shared operating-point builders fea_p_case1..4 in app.py; fea_export.py holds the file generators.
